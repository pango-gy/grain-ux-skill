...
`grain` は、木目のように素材がもつ自然な方向や質感を意味します。"with the grain" は、その自然な流れに逆らわずに進むという意味です。このスキルは、人が実際に考え、読み、判断し、信頼し、失敗から回復する流れに沿ってUXを判断するため、この名前を使っています。
...
デフォルトでは、`skills` CLI はプロジェクトスキルを `./.agents/skills/grain` にインストールします。この共有ディレクトリは、Codex や Gemini CLI などの universal agent が使うプロジェクト単位の場所です。Claude Code のように専用のプロジェクトスキルディレクトリを持つ agent は、そのディレクトリがプロジェクトに存在するときに symlink または copy を受け取ります。

対応するすべての agent ディレクトリに実体 copy を作成したい場合:

```bash
npx skills add pango-gy/grain-ux-skill --agent '*' --skill '*' -y --copy
```

更新するときは add コマンドを再実行してください。
...
このコマンドはインストール済みの skill ディレクトリを削除します。プロジェクトで `skills-lock.json` を使っている場合、CLI が復元/更新用の source metadata を残すことがあるため、削除後にそのファイルを確認してください。
